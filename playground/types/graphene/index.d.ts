...
export const _false: number;
export const _true: number;
/**
...
/**
 * A #graphene_box_t with both the minimum and maximum vertices set at (0
 * , 0, 0).
 * The returned value is owned by Graphene and should not be modified or 
 * freed.
 */
...
init_from_points(n_points: number, points: Point3D[]): Box;
init_from_vec3(min: Vec3 | null, max: Vec3 | null): Box;
init_from_vectors(n_vectors: number, vectors: Vec3[]): Box;
...
export class Frustum  {constructor(config?: properties);
...
export class Plane  {constructor(config?: properties);
static alloc(): Plane;
...
export class Point3D  {constructor(config?: properties);
...
export class Quad  {constructor(config?: properties);
static alloc(): Quad;
bounds(): [Rect];
...
export class Ray  {constructor(config?: properties);
static alloc(): Ray;
equal(b: Ray): boolean;
free(): void;
...
export class Simd4F  {constructor(config?: properties);
readonly x: number;
readonly y: number;
readonly z: number;
readonly w: number;
}
export class Simd4X4F  {constructor(config?: properties);
readonly x: Simd4F;
readonly y: Simd4F;
readonly z: Simd4F;
readonly w: Simd4F;
...
export class Sphere  {constructor(config?: properties);
static alloc(): Sphere;
contains_point(point: Point3D): boolean;
distance(point: Point3D): number;
...
init_from_points(n_points: number, points: Point3D[], center: Point3D | null): Sphere;
init_from_vectors(n_vectors: number, vectors: Vec3[], center: Point3D | null): Sphere;
is_empty(): boolean;
translate(point: Point3D): [Sphere];
}
export class Triangle  {constructor(config?: properties);
...
export class Vec2  {constructor(config?: properties);
static alloc(): Vec2;
add(b: Vec2): [Vec2];
divide(b: Vec2): [Vec2];
dot(b: Vec2): number;
...
export class Vec4  {constructor(config?: properties);
static alloc(): Vec4;
add(b: Vec4): [Vec4];
divide(b: Vec4): [Vec4];
dot(b: Vec4): number;
